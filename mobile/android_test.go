...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.olink", new NodeConfig());
			node.start();
	...
	// signatures too. Regression test for go-olink issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altoolinker
	...
	workspace, err := ioutil.TempDir("", "olink-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/olink/olinktest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.olink", "github.com/olink/go-olink/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "olink.aar"), "olink.aar")

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "olink", "olinktest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
