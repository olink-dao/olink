...
type wallet struct {
	...
	deriveChain     olink.ChainStateReader // Blockchain state reader to discover used account with
...
// chain state reader.
func (w *wallet) SelfDerive(bases []accounts.DerivationPath, chain olink.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
