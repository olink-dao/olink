...
import (
	...
	"github.com/olink/go-olink/accounts/external"
	"github.com/olink/go-olink/accounts/keystore"
	"github.com/olink/go-olink/accounts/scwallet"
	"github.com/olink/go-olink/accounts/usbwallet"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of olink is "olink". If no
...
func (c *Config) NodeName() string {
	...
	if name == "olink" || name == "olink-testnet" {
		name = "Geth"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "olink" instances.
...
func (c *Config) ResolvePath(path string) string {
	...
	// by olink 1.4 are used if they exist.
	if warn, isOld := isOldGethResource[path]; isOld {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGethResourceWarning, "Using deprecated resource file %s, please move this file to the 'olink' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-olink-keystore")
		ephemeral = keydir
	}
