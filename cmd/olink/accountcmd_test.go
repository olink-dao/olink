...
func TestAccountListEmpty(t *testing.T) {
	olink := runGeth(t, "account", "list")
	olink.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	olink := runGeth(t, "account", "list", "--datadir", datadir)
	defer olink.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	olink := runGeth(t, "account", "new", "--lightkdf")
	defer olink.ExpectExit()
	olink.Expect(`
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	olink := runGeth(t, "account", "import", keyfile, "-password", passwordFile)
	defer olink.ExpectExit()
	olink.Expect(expected)
}
...
func TestAccountUpdate(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	olink := runGeth(t, "account", "update",
...
func TestWalletImport(t *testing.T) {
	olink := runGeth(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer olink.ExpectExit()
	olink.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(olink.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(olink.StderrText(), m) {
...
// https://github.com/olink/go-olink/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	olink.SetTemplateFunc("keypath", func(file string) string {
