...
func (g *olinkrpc) killAndWait() {
	g.olink.Kill()
	g.olink.WaitExit()
}

func (g *olinkrpc) callRPC(result interface{}, method string, args ...interface{}) {
	if err := g.rpc.Call(&result, method, args...); err != nil {
		g.olink.Fatalf("callRPC %v: %v", method, err)
	}
}

func (g *olinkrpc) addPeer(peer *olinkrpc) {
	g.olink.Logf("%v.addPeer(%v)", g.name, peer.name)
	enode := peer.getNodeInfo().Enode
	peerCh := make(chan *p2p.PeerEvent)
	sub, err := g.rpc.Subscribe(context.Background(), "admin", peerCh, "peerEvents")
	if err != nil {
		g.olink.Fatalf("subscribe %v: %v", g.name, err)
	}
	...
	select {
	case ev := <-peerCh:
		g.olink.Logf("%v received event: type=%v, peer=%v", g.name, ev.Type, ev.Peer)
	case err := <-sub.Err():
		g.olink.Fatalf("%v sub error: %v", g.name, err)
	case <-timeout:
		g.olink.Error("timeout adding peer after", dur)
	}
}

// Use this function instead of `g.nodeInfo` directly
func (g *olinkrpc) getNodeInfo() *p2p.NodeInfo {
	if g.nodeInfo != nil {
		return g.nodeInfo
	}
...
func (g *olinkrpc) waitSynced() {
	...
	if err != nil {
		g.olink.Fatalf("%v syncing: %v", g.name, err)
	}
	defer sub.Unsubscribe()
	timeout := time.After(4 * time.Second)
	select {
	case ev := <-ch:
		g.olink.Log("'syncing' event", ev)
		syncing, ok := ev.(bool)
		if ok && !syncing {
			break
		}
		g.olink.Log("Other 'syncing' event", ev)
	case err := <-sub.Err():
		g.olink.Fatalf("%v notification: %v", g.name, err)
		break
	case <-timeout:
		g.olink.Fatalf("%v timeout syncing", g.name)
		break
	}
}

func startGethWithIpc(t *testing.T, name string, args ...string) *olinkrpc {
	g := &olinkrpc{name: name}
	args = append([]string{"--networkid=42", "--port=0", "--nousb"}, args...)
	t.Logf("Starting %v with rpc: %v", name, args)
	g.olink = runGeth(t, args...)
	// wait before we can attach to it. TODO: probe for it properly
	time.Sleep(1 * time.Second)
	var err error
	ipcpath := filepath.Join(g.olink.Datadir, "olink.ipc")
...
func startLightServer(t *testing.T) *olinkrpc {
	datadir := initGeth(t)
...
func startClient(t *testing.T, name string) *olinkrpc {
	datadir := initGeth(t)
